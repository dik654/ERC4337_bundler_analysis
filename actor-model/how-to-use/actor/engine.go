...
// producer는 Receiver를 리턴하는 함수
// actor instance를 생성하는 역할
...
func NewEngine(config EngineConfig) (*Engine, error) {
	// 엔진 인스턴스 생성
	e := &Engine{}
	// Registry는 각 actor instance에 대한 참조를 저장
	// actor의 ID를 통해 특정 액터를 조회
...
func (e *Engine) send(pid *PID, msg any, sender *PID) {
	...
	// eventStream actor에게 메세지를 보내어 구독중인 모든 actor에게 이벤트 전송
	if e.remote == nil {
